...
//       {(userLoading && repoLoading) && <p>Loading...</p>}

//       {userError && <p>Users Error: {userError.message}</p>}
//       {repoError && <p>repos Error: {repoError.message}</p>}

//       {(userData && inputValue !== '') &&
//         <ul className='w-full px-4'>
//           <p className='text-custom_grey text-xs p-2'>Owners</p>
//           {userData.search.nodes.map((user: { name: string; login: string }, i: number) => (
//             <li key={`${user.name}_${user.login}_${i}`} className='flex items-center justify-between hover:bg-custom_light_grey px-2 py-1.5 rounded-lg'>
...
//             <li key={`${repo.name}_${repo.owner.login}`} className='flex items-center justify-between hover:bg-custom_light_grey px-2 py-1.5 rounded-lg'>
//               <div className='flex items-center gap-2'>
//                 <FontAwesomeIcon icon={faBookBookmark} />
//                 <div className='flex'>
//                   <p className=''>{repo.owner.login}</p>
//                   <p>/</p>
//                   <p>{repo.name}</p>
//                 </div>
//               </div>
        ...
        <div className="relative" id="search-container">
          ...
          {isModalOpen && (
            <>
              ...
              <div
                id="search-results"
                className="absolute -top-4 left-0 right-0 mt-1 bg-white rounded-lg shadow-lg max-h-96 overflow-y-auto z-50"
                onClick={(e) => {
