...
const Contributions = () => {
  ...
  const contributionCalendar = contributions?.contributionCalendar || null;
  ...
  return (
    <div className=''>
      ...
      <main className='border border-custom_border_grey rounded px-3 py-2 text-xs w-full'>
        <table className='flex items-start overflow-x-auto'>
          ...
          <tbody className='flex gap-0.5'>
            {contributionCalendar?.weeks.map((week, i) => (
              <tr key={i} className='flex flex-col gap-0.5'>
        ...
        <div className='flex justify-between text-custom_grey px-8 pt-2 pb-1'>
